...
```{r, include = FALSE}
compas_seldonian <- compas %>%
  dplyr::select(c(race, sex, age, age_cat, marital_status, juv_offense, priors_count, is_recid))

write_csv(compas_seldonian, "/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian.csv")
```
...
f_orig = "/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian.csv"
...
Let's also limit the analysis to just Black and White defendants. We may consider other races in future analysis. 

```{python}
outdf_bw = outdf.drop(columns=["Asian", "race_Other", "Hispanic", "NativeAmerican"])
```
...
output_path_data="/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian_numeric_clean.csv"

outdf_bw.to_csv(output_path_data,index=False,header=False)
...
output_path_metadata="/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/metadata_compas_seldonian.json"

metadata_dict = {
  "regime":"supervised_learning",
  "sub_regime":"classification",
  "all_col_names":list(outdf_bw.columns),
...
1. Equalized odds: 'abs((FNR | [Black]) - (FNR | [White])) + abs((FPR | [Black]) - (FPR | [White])) <= 0.2'
2. Equal opportunity: 'abs((FNR | [Black]) - (FNR | [White])) <= 0.1'
3. Predictive equality: 'abs((FPR | [Black]) - (FPR | [White])) <= 0.1'
...
Therefore, the Seldonian ML problem can be formulated as: using gradient descent on a logistic regression model, minimize the logistic loss, subject to either $g_1$, $g_2$, $g_3$, or both $g_2$ and $g_3$, with $\delta_1, \delta_2, \delta_3 = 0.05$.
...
spec_save_name = os.path.join(save_dir,f'compas_{constraint_name}_{epsilon}_spec.pkl')

...
ggplot(data = compas_final, 
  ...
  theme_minimal() +
  facet_wrap(~race) +
  labs(title = "Juveline Offense by Race",
       x = "Juveline Offense",
       fill = "Juveline Offense") 
```
...
There are now `r count(train)`	observations in the train set and `r count(test)` observations in the test set. Let's ensure that the distribution of the response variable is preserved in each set. There split appears to be stratified so there is no concern. 
...
Next, let's test the Seldonian framework on this data set in an aim to achieve more equitable results by enforcing fairness constraints. 
...
```{r, include = FALSE}
compas_seldonian_bw <- compas %>%
  dplyr::select(c(race, sex, age, age_cat, marital_status, juv_offense, priors_count, is_recid)) %>%
  filter(race %in% c('African-American', 'Caucasian'))

write_csv(compas_seldonian_bw, "/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian_bw.csv")
```
...
ct = ColumnTransformer([('c',OneHotEncoder(),['race', 'sex', 'age_cat', 'marital_status']), ('n',StandardScaler(),['age','priors_count'])])
...
outdf.rename(columns={'c__race_African-American':'Black', 'c__race_Caucasian':'White', 'c__sex_Female':'Female', 'c__sex_Male':'Male', 'c__age_cat_25 - 45':'25_45', 'c__age_cat_Greater than 45':'Greater_than_45', 'c__age_cat_Less than 25':'Less_than_25', 'c__marital_status_Divorced':'Divorced', 'c__marital_status_Married':'Married', 'c__marital_status_Separated':'Separated', 'c__marital_status_Significant Other':'Significant_Other', 'c__marital_status_Single':'Single', 'c__marital_status_Unknown':'marital_status_Unknown', 'c__marital_status_Widowed':'Widowed', 'c__juv_offense_0':'juv_offense_0', 'c__juv_offense_1':'juv_offense_1', 'n__age':'age', 'n__priors_count':'priors_count' },
inplace=True)
...
output_path_data="/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian_numeric_clean_bw.csv"
...
output_path_metadata="/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/metadata_compas_seldonian_bw.json"
...
1. Equalized odds: 'abs((FNR | [Black]) - (FNR | [White])) + abs((FPR | [Black]) - (FPR | [White])) <= $\epsilon$'
2. Equal opportunity: 'abs((FNR | [Black]) - (FNR | [White])) <= $\epsilon$'
3. Predictive equality: 'abs((FPR | [Black]) - (FPR | [White])) <= $\epsilon$'
...
if constraint_name == "equalized_odds":
  ...
  #constraint_strs = [f'abs((FPR | [Black]) - (FPR | [White])) <= {epsilon}'] 
deltas = [0.05]
...
We are now ready to run the Seldonian algorithm using the spec file generated in the previous step. The code below modifies some defaults of the spec object that we created and then runs the Seldonian algorithm using the modified spec object.
...
# load the spec file
#specfile = "/home/dasienga24/Statistics-Senior-Honors-Thesis/Python/COMPAS Application/compas_predictive_equality_0.4_spec.pkl"
specfile = '/home/dasienga24/Statistics-Senior-Honors-Thesis/Python/COMPAS Application/spec.pkl'
...
We got a solution! Therefore, we are 95% confident that the equalized odds -- 'abs((FNR | [Black]) - (FNR | [White])) + abs((FPR | [Black]) - (FPR | [White])) -- <= 0.2'. Let's now analyze this solution in more depth.

## Obtain the Predictive Values

```{python}
SA.cs_result["candidate_solution"]
```

$\\$

Let's store this coefficients. The first of which is the intercept.

```{python}
coefficients = SA.cs_result["candidate_solution"]
```
...
Let's also separate the predictor variables from the race and output variables.
...
Now, let's compute the predictive values.
...
```{python}
seldonian_results = pd.DataFrame({'is_recid': y_outdf, 'pred': predicted_probabilities})
...
Finally, let's obtain a binary risk prediction. 
...
# create risk column
risk = np.where(predicted_probabilities >= threshold, 1, 0)

# add risk column to dataframe
seldonian_results['risk'] = risk
```
...
seldonian_results.to_csv("/home/dasienga24/Statistics-Senior-Honors-Thesis/Data Sets/COMPAS/compas_seldonian_results_bw.csv", index=False)
```
...
```{r}
tally(data = seldonian_results, risk ~ is_recid)
```
...
This means that ~32% of defendants are misclassified in the incorrect recidivism prediction. However, the real determinant of this model's performance is the difference in FPR v FNR for Black v White defendants.  
...
seldonian_results <- seldonian_results %>%
  mutate(race = ifelse(Black == 1, 'Black', 'White'),
         pred_risk = ifelse(risk == 0, 'Low', 'High')) 
...
In conclusion, the Seldonian algorithm still exhibits discrepancies between the FPR and FNR for Black v White defendants, which perhaps is unavoidable because of the role that race plays in proxy variables. However, there was barely any accuracy trade-off, and we saw an increase in FNR for both races, accompanied by a decrease in FPR for both races. There is still inequality, but everyone is just a little bit happier.

# Misc

Would be interesting to see what enforcing a smaller $\epsilon$ (tighter constraint) would do!
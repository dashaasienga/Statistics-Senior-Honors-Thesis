...
```{r}
# define a linear combination of predictors as desired
linear_combination = - 0.34 - 16 * compas_sim_balanced$age + ifelse(compas_sim_balanced$prior_offense == 1, 600, 0)
...
There are 2733 defendants who did not recidivate in this data set. 

```{r}
count(compas_sim_balanced_4$is_recid_sim == 0)
```

$\\$

There are 2267 defendants who recidivated in this data set. 

```{r}
count(compas_sim_balanced_4$is_recid_sim == 1)
```
...
set.seed(93)
# define a linear combination of predictors as desired
error <- rnorm(n = 5000, mean = 150, sd = 120)
...
There are 2204 defendants who did not recidivate in this data set. 

```{r}
count(compas_sim_balanced_5$is_recid_sim == 0)
```

$\\$

There are 2796 defendants who recidivated in this data set. 

```{r}
count(compas_sim_balanced_5$is_recid_sim == 1)
```
...
```{r}
set.seed(93)
compas_sim_balanced_5 <- compas_sim_balanced_5[sample(nrow(compas_sim_balanced_4), 
                                                      5000, 
                                                      replace = FALSE),]
```
...
There are 2380 defendants who did not recidivate in this data set. 

```{r}
count(compas_sim_balanced_6$is_recid_sim == 0)
```

$\\$

There are 2620 defendants who recidivated in this data set. 

```{r}
count(compas_sim_balanced_6$is_recid_sim == 1)
```
...
This data set has 93.12% accuracy.
...
Some discrepancy is preserved, and in the same direction as previous analyses, although not of the same magnitude. The discrimination statistic is ~0.09.

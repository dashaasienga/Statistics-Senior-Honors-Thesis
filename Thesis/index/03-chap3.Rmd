...
Of the 9387 total defendants from Broward County, Florida, in 2013 and 2014, `r round(100*count(clean_compasdata$sex == "Male")/nrow(clean_compasdata),1)`% were male. About `r round(100*count(clean_compasdata$age_cat == "Less than 25")/nrow(clean_compasdata),1)`% were < 25 years old, `r round(100*count(clean_compasdata$age_cat == "25 - 45")/nrow(clean_compasdata),1)`% were between 25 and 45 years old, and 
`r round(100*count(clean_compasdata$age_cat == "Greater than 45")/nrow(clean_compasdata),1)`% were > 45 years old. The majority of defendants were single (`r round(100*count(clean_compasdata$marital_status == "Single")/nrow(clean_compasdata),1)`%), with the next largest categories being married (`r round(100*count(clean_compasdata$marital_status == "Married")/nrow(clean_compasdata),1)`%) and divorced (`r round(100*count(clean_compasdata$marital_status == "Divorced")/nrow(clean_compasdata),1)`%). The other levels were significant other, separated, unknown, and widowed, respectively. Finally, `r round(100*count(clean_compasdata$juv_offense == 0)/nrow(clean_compasdata),1)`% of defendants had no record of juvenile offenses. The remaining `r round(100*count(clean_compasdata$juv_offense == 1)/nrow(clean_compasdata),1)`% ranged from having one juvenile offense to 21 juvenile offenses on record, with a median of one, a mean of 1.96, and the middle 50% recording one or two juvenile offenses.  
...
ggplot(mapping = aes(x = is_recid)) +
  ...
  labs(title = "Recidivism",
       x = "Recommitted a Crime Within Two Years",
       y = "Number of Defendants")
```

### Protected Attribute {#protectedattribute}

Finally, the protected attribute in this analysis is race. Most of the defendants were African-American (`r round(100*count(clean_compasdata$race == "African-American")/nrow(clean_compasdata),1)`%) and Caucasian (`r round(100*count(clean_compasdata$race == "Caucasian")/nrow(clean_compasdata),1)`%), with only `r round(100*count(clean_compasdata$race == "Hispanic")/nrow(clean_compasdata),1)`% Hispanic, `r round(100*count(clean_compasdata$race == "Asian")/nrow(clean_compasdata),1)`% Asian, and finally, `r round(100*count(clean_compasdata$race == "Native American")/nrow(clean_compasdata),1)`% Native American. The remaining `r round(100*count(clean_compasdata$race == "Other")/nrow(clean_compasdata),1)`% of defendants identified as 'Other'. 
...
```{r ch3fig5, fig.align='center', fig.cap="Distribution of Age (panel A) and Number of Prior Offenses (panel B) by Recidivism Status among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 6, fig.height = 2.75, echo = FALSE}
...
```{r ch3fig6, fig.align='center', fig.cap="Conditional Distribution of Age (panel A) and Juvenile Offense (panel B) by Recidivism Status among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 4.5, fig.height = 4.25, echo = FALSE}
...
new1 <- clean_compasdata %>%
  mutate(is_recid = ifelse(is_recid == 0, "No", "Yes"),
         age_cat = factor(age_cat, levels = c("Greater than 45", "25 - 45", "Less than 25"))) %>%
  ggplot(mapping = aes(x = as.factor(is_recid), fill = age_cat)) +
...
Recall that COMPAS decile scores of one to four are mapped as 'low' risk, five to seven as 'medium' risk, and eight to ten as 'high' risk. The median decile score for the entire data set was four, and Figure \@ref(fig:ch3fig8) illustrates that the COMPAS tool classified more than half of the defendants as low risk. In particular, the tool classified `r round(100*count(clean_compasdata$score_text == "Low")/nrow(clean_compasdata),1)`% as low risk and `r round(100*count(clean_compasdata$score_text == "High")/nrow(clean_compasdata),1)`% as high risk, with the remaining `r round(100*count(clean_compasdata$score_text == "Medium")/nrow(clean_compasdata),1)`% as medium risk. These predictions align with expectations since most defendants did not recommit a crime within the two-year time window, as observed in Section \@ref(response). 


```{r ch3fig8, fig.align='center', fig.cap="Distribution of COMPAS Tool Decile Scores among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 6, fig.height = 3, echo = FALSE}
...
```{r ch3fig9, fig.align='center', fig.cap="Distribution of COMPAS Tool Risk Scores among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 5, fig.height = 2.5, echo = FALSE}
order <- c("Low", "Medium", "High")
...
compas %>%
  ...
  kable(digits = 2,
        caption = "The Number of Defendants who were Labeled Higher/Lower Risk by the COMPAS Tool Stratified by Recidivism Status among 9387 Defandants in Broward County Florida, 2013-2014",
        booktabs = TRUE)
```
...
compas %>%
  ...
  rename("Recidivism Status" = Recidivism,
         "Predicted Risk" = Risk) %>%
  kable(digits = 2,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by the COMPAS Tool Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014",
        booktabs = TRUE)
```


```{r ch3fig11, fig.align='center', fig.cap="Distribution of COMPAS Tool Decile Scores Stratified by Race among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 6, fig.height = 2.5, echo = FALSE}
compas %>%
...
```{r ch3fig13, fig.align='center', fig.cap="Distribution of Recidivism Prevalence Stratified by Race among 9387 Defandants in Broward County Florida, 2013-2014", warning = FALSE, message = FALSE, fig.width = 6, fig.height = 2.5, echo = FALSE}
compas_final %>%
...
test2 %>%
  ...
  rename("Recidivism Status" = Recidivism,
         "Predicted Risk" = Risk) %>%
  kable(digits = 2,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by Logistic Regression Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014",
        booktabs = TRUE)
```
...
seldonian_results %>%
  ...
  kable(digits = 2,
        booktabs = TRUE,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by a Seldonian Algorithm ($\\epsilon$ = 0.2) Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014")
```
...
seldonian_results %>%
  ...
  kable(digits = 2,
        booktabs = TRUE,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by a Seldonian Algorithm ($\\epsilon$ = 0.1) Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014")
```
...
seldonian_results %>%
  ...
  kable(digits = 2,
        booktabs = TRUE,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by a Seldonian Algorithm ($\\epsilon$ = 0.05) Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014")
```
...
seldonian_results %>%
  ...
  kable(digits = 2,
        booktabs = TRUE,
        caption = "The Percent of Defendants who were Incorrectly Labeled Higher/Lower Risk by a Seldonian Algorithm ($\\epsilon$ = 0.01) Stratified by Recidivism Status and Race among 9387 Defandants in Broward County Florida, 2013-2014")
```

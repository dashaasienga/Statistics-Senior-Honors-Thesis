...
Finally, the protected attribute in this analysis is race. Most of the defendants are African-American (`r round(100*count(clean_compasdata$race == "African-American")/nrow(clean_compasdata),1)`%) and Caucasian (`r round(100*count(clean_compasdata$race == "Caucasian")/nrow(clean_compasdata),1)`%), with only `r round(100*count(clean_compasdata$race == "Hispanic")/nrow(clean_compasdata),1)`% Hispanic, `r round(100*count(clean_compasdata$race == "Asian")/nrow(clean_compasdata),1)`% Asian, and finally, `r round(100*count(clean_compasdata$race == "Native American")/nrow(clean_compasdata),1)`% Native American. The remaining `r round(100*count(clean_compasdata$race == "Other")/nrow(clean_compasdata),1)`% of defendants identify as 'Other'. 
...
Recall that COMPAS decile scores of one to four are mapped as 'low' risk, five to seven as 'medium' risk, and eight to ten as 'high' risk. The median decile score for the entire data set is four, and Figure \@ref(fig:ch3fig8) illustrates that the COMPAS tool classifies more than half of the defendants as low risk. In particular, the tool classifies `r round(100*count(clean_compasdata$score_text == "Low")/nrow(clean_compasdata),1)`% as low risk and `r round(100*count(clean_compasdata$score_text == "High")/nrow(clean_compasdata),1)`% as high risk, with the remaining `r round(100*count(clean_compasdata$score_text == "Medium")/nrow(clean_compasdata),1)`% as medium risk. These predictions align with expectations since most defendants did not recommit a crime within the two-year time window, as observed in Section \@ref(response). 

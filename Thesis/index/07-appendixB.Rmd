# Quasi-Seldonian Linear Regression Theoretical Considerations and Python Implementation {#appendix-b}

The theoretical considerations and Python implementation discussed in this Appendix are either reproduced or adapted from @aisafety. Full Chapter 2 code available through GitHub: https://github.com/dashaasienga/Statistics-Senior-Honors-Thesis/blob/main/Thesis/index/02-chap2.Rmd.

$\\$

\noindent The `reticulate` package is useful for setting up the correct Python environment within RStudio.
...
\noindent Python packages not already pre-installed need to be imported into the `reticulate` package first before being imported into the Python environment, as illustrated using the  `sklearn` package below.
...
\noindent The `tinv` function returns the inverse of `Student's t` CDF using the `nu` degrees of freedom for the corresponding probabilities `p`.
...
def ttestUpperBound(v, delta):
    n  = v.size
    res = v.mean() + stddev(v) / math.sqrt(n) * tinv(1.0 - delta, 
    n - 1)
    return res
```
...
def predictTTestUpperBound(v, delta, k):
    ...
    res = v.mean() + 2.0 * stddev(v) / math.sqrt(k) * tinv(1.0 - delta, 
    k - 1)
    return res
```
...
\noindent The `generateData` function samples data as desired for the specific problem.
...
\noindent The `fHat` function specifies the primary objective: to minimize the sample mean squared error. The attempt is to maximize $\hat{f}$; the negative sample mean squared error is returned, so that maximizing $\hat{f}$ corresponds to minimizing the mean squared error.

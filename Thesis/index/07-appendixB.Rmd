...
The theoretical considerations and Python implementation discussed in this Appendix are either reproduced or adapted from @aisafety.
...
\noindent The `fHat` function specifies the primary objective: to minimize the sample mean squared error. Because the attempt is to maximize $\hat{f}$, however, the negative sample mean squared error is returned, so that maximizing $\hat{f}$ corresponds to minimizing the mean squared error.
